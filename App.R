...
Measurements <- read.csv(file = "https://raw.githubusercontent.com/kennethsamcs-star/S3729C/refs/heads/main/TUGbalanceData.csv", header = TRUE, sep = ",")
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput(
        ...
        label = "Y-axis:",
        choices = c(
          "Total Times Taken" = "total_time_taken",
...
server <- function(input, output, session) {
  ...
  output$measurementstable <- renderDataTable({
    brushedPoints(Measurements, brush = input$plot_brush) %>%
      select(name, age, height, weight, gender, total_time_taken, time_seated_to_full_extension, time_around_cone, time_turn_to_sit, time_to_sitting_down, first_foot_walk, last_foot_walk, first_foot_around_cone, last_foot_turn_before_sitting, risk_level, s_open_c90area, u_closed_c90area)
  })
